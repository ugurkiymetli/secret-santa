    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === "Account not claimed. Please claim your account.") {
          ...
          setError(data.error) // Backend errors might still be English, ideally backend should also be localized or we map errors here.
        }
      } else {
    ...
    <Card className="w-[350px]">
      <CardHeader className="space-y-1">
        ...
        <CardDescription className="text-center">
          {isLogin ? "Giriş yapmak için bilgilerinizi girin" : "Hesabınızı doğrulayın ve şifre belirleyin"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit}>
          ...
          <Button className="w-full mt-6" type="submit" disabled={loading}>
            {loading ? "Yükleniyor..." : (isLogin ? "Giriş Yap" : "Hesabı Doğrula")}
          </Button>
        </form>
      </CardContent>
      <CardFooter className="flex justify-center">
        ...
        >
          {isLogin ? "Hesabınızı doğrulamanız mı gerekiyor?" : "Zaten hesabınız var mı?"}
        </Button>
      </CardFooter>
    </Card>
  )
